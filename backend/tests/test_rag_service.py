...
def rag_service(mock_vector_store):
    ...
    with patch("app.services.rag_service.PGVector", return_value=mock_vector_store), \
...
def test_ingest_document_pdf_aload(rag_service):
    ...
    with patch("app.services.rag_service.PyPDFLoader", return_value=mock_loader) as mock_pdf_loader:
        # Mock text splitter to just return the same docs
        with patch("app.services.rag_service.RecursiveCharacterTextSplitter") as mock_splitter_cls:
...
def test_ingest_document_text_load(rag_service):
    ...
    with patch("app.services.rag_service.TextLoader", return_value=mock_loader) as mock_text_loader:
        with patch("app.services.rag_service.RecursiveCharacterTextSplitter") as mock_splitter_cls:
            mock_splitter = mock_splitter_cls.return_value
            mock_splitter.split_documents.return_value = [mock_doc]
...
def test_ingest_document_metadata(rag_service):
    ...
    with patch("app.services.rag_service.TextLoader", return_value=mock_loader), \
         patch("app.services.rag_service.RecursiveCharacterTextSplitter") as mock_splitter_cls:
